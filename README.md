...
## Chatbot (kural tabanlı)

- Sağ altta yüzen sohbet botu; yapay zekâ kullanmaz, cevapları tamamen veritabanından alır. 3 dilde çalışır.
- Kurallar `chat_rules` tablosunda (dil başına `locale`): her kural bir `question` (hızlı öneri olarak da görünür), `keywords` listesi ve `answer` içerir.
- Akış: `listChatRulesPublic(locale)` → `src/components/ChatWidget.tsx` (istemci tarafında anahtar kelime eşleştirme; eşleşme yoksa `dict.chat.fallback`).
- Yönetim: **Admin → İçerik → Chatbot** ekranından kural ekle/düzenle/sil (aktif admin diline yazılır). Hiçbir env değişkeni gerekmez.
