// DB'den GELMEYEN sabit UI metinleri (TR/EN/DE).
// DB içeriği (başlıklar, hero, hizmet metinleri vb.) page_sections + tablolardan gelir.
import type { Locale } from "./config";

const tr = {
  nav: {
    home: "Anasayfa",
    about: "Hakkımızda",
    services: "Hizmetler",
    portfolio: "Portfolyo",
    blog: "Blog",
    contact: "İletişim",
    allServices: "Tüm hizmetler",
    getQuote: "Teklif Al",
    menuOpen: "Menüyü aç",
    menuClose: "Menüyü kapat",
    servicesHint: "Tek bir hizmet mi, paket mi? Birlikte karar verelim.",
  },
  meta: {
    keywords: [
      "dijital pazarlama ajansı",
      "Google Ads ajansı",
      "Meta Ads",
      "SEO ajansı",
      "mobil uygulama geliştirme",
      "SaaS geliştirme",
      "sosyal medya yönetimi",
      "performans pazarlama",
    ],
  },
  common: {
    skipToContent: "İçeriğe geç",
    planCall: "Görüşme Planla",
    getQuoteFree: "Ücretsiz Teklif Al",
  },
  footer: {
    quickLinks: "Hızlı Linkler",
    services: "Hizmetler",
    contact: "İletişim",
    rights: "Tüm hakları saklıdır.",
    founder: "Kurucu",
    city: "İstanbul",
  },
  notFound: {
    title: "Aradığınız sayfa bulunamadı",
    desc: "Sayfa taşınmış ya da hiç var olmamış olabilir. Aşağıdaki bağlantılardan devam edebilirsiniz.",
    home: "Anasayfaya Dön",
    contact: "Bize Yazın",
  },
  contactForm: {
    name: "Ad Soyad",
    namePlaceholder: "Adınız Soyadınız",
    email: "E-posta",
    emailPlaceholder: "ornek@sirket.com",
    phone: "Telefon (opsiyonel)",
    phonePlaceholder: "+90 5XX XXX XX XX",
    service: "İlgilendiğiniz Hizmet",
    serviceSelect: "Seçiniz...",
    serviceOther: "Diğer / Emin değilim",
    message: "Mesajınız",
    messagePlaceholder: "Projeniz, hedefleriniz ve beklentileriniz hakkında kısaca yazın...",
    privacy: "Gönder'e tıklayarak gizlilik politikamızı kabul etmiş olursunuz.",
    submit: "Mesajı Gönder",
    submitting: "Gönderiliyor...",
    successTitle: "Mesajınız bize ulaştı!",
    successDesc: "En geç 48 saat içinde size dönüş yapacağız.",
    successAgain: "Yeni mesaj gönder",
    errName: "Lütfen adınızı girin (en az 2 karakter).",
    errEmail: "Geçerli bir e-posta adresi girin.",
    errPhone: "Geçerli bir telefon numarası girin.",
    errService: "Bir hizmet seçin.",
    errMessage: "Lütfen daha detaylı bir mesaj yazın (en az 20 karakter).",
    errSubmit: "Gönderim başarısız. Lütfen tekrar deneyin.",
  },
  serviceDetail: {
    crumbHome: "Anasayfa",
    crumbServices: "Hizmetler",
    eyebrow: "Hizmet",
    getQuote: "Bu Hizmet İçin Teklif Al",
    examples: "Bu Alandaki Örnekler",
    idealFor: "Kimler için ideal?",
    approachEyebrow: "Yaklaşımımız",
    approachTitle: "Bu hizmeti nasıl ele alıyoruz",
    scopeEyebrow: "Kapsam",
    scopeTitle: "Bu hizmet neleri içerir?",
    scopeDesc: "Sözleşmede yer almayan vaatler vermiyoruz.",
    serviceScope: "Hizmet kapsamı",
    outcomes: "Çıktılar",
    expectedResults: "Beklenebilir sonuçlar",
    processEyebrow: "Süreç",
    processTitle: "İş başı yapışımızdan ilk sonuca",
    toolsEyebrow: "Araçlar",
    toolsTitle: "Kullandığımız stack",
    faqEyebrow: "Sık Sorulanlar",
    relatedEyebrow: "İlgili Hizmetler",
    backToAll: "Tüm hizmetlere dön",
    breadcrumbAria: "Yol haritası",
    processDesc: "Birlikte çalışmaya başladığımız andan teslim/yayın anına kadar geçtiğimiz adımlar.",
    toolsDesc: "Bu hizmet için günlük olarak kullandığımız platformlar.",
    relatedTitle: "Bu hizmetle birlikte tercih edilenler",
    relatedDesc: "Tek başına alabilirsiniz, ama birlikte sinerji çok daha güçlü.",
  },
  contactPage: {
    socialMedia: "Sosyal Medya",
    dmNote: "DM kanallarımız da aktif — ama detaylı projeler için e-posta önerilir.",
    mapsOpen: "Google Maps'te Aç",
    officeTitle: "Ofisimiz",
    officeMapTitle: "Ofis konumu",
  },
  dashboard: {
    label: "Canlı",
    title: "Kampanya Paneli",
    revenue: "Gelir",
    roas: "ROAS",
    conversions: "Dönüşüm",
    cpa: "Edinme Maliyeti",
    traffic: "Performans · son 90 gün",
    channels: "Kanal dağılımı",
  },
  compare: {
    eyebrow: "Farkımız",
    title: "Ayrı ajanslar yerine tek sinir sistemi",
    description:
      "Tipik kurguda her ajans yalnızca kendi performansına bakar. Biz tek takım olduğumuz için kanallar arası kayıp sıfıra iner.",
    them: "Tipik ajans kurgusu",
    us: "True EDigital",
    rows: [
      { them: "Reklam, SEO, sosyal için ayrı ajanslar", us: "Tüm kanallar tek elde" },
      { them: "Kanallar arası kopukluk, bilgi silosu", us: "Tek brief, kanallar arası sıfır kayıp" },
      { them: "Gösterim ve beğeni raporları", us: "Satış, lead ve ROAS odaklı raporlama" },
      { them: "Junior'a devredilen hesaplar", us: "Kurucu her projede bizzat çalışıyor" },
      { them: "Yavaş iterasyon, aylık değişim", us: "48 saatte test, haftalık optimizasyon" },
    ],
  },
  blog: {
    nav: "Blog",
    eyebrow: "Blog",
    title: "Dijital büyüme üzerine notlar",
    description:
      "Reklam, SEO ve dönüşüm optimizasyonu üzerine uygulanabilir yazılar. Teori değil, sahadan çıkan yöntemler.",
    readingSuffix: "dk okuma",
    readMore: "Yazıyı oku",
    empty: "Henüz yayınlanmış yazı yok.",
    backToList: "Tüm yazılara dön",
    related: "İlgili yazılar",
    shareTitle: "Bu yazıyı paylaş",
    ctaTitle: "Bu konuyu markanız için konuşalım",
    ctaText: "Ücretsiz 30 dakikalık keşif görüşmesinde mevcut kanallarınızı inceleyip somut bir plan çıkarıyoruz.",
  },
  caseStudy: {
    client: "Müşteri",
    duration: "Süre",
    headline: "Öne çıkan sonuç",
    challenge: "Başlangıç durumu",
    approach: "Yaklaşımımız",
    processEyebrow: "Süreç",
    processTitle: "Nasıl ilerledik",
    resultsEyebrow: "Öne çıkanlar",
    resultsTitle: "Projenin künyesi",
    deliverablesEyebrow: "Teslim edilenler",
    deliverablesTitle: "Proje kapsamında neler üretildi",
    otherEyebrow: "Diğer vakalar",
    otherTitle: "Benzer çalışmalar",
    backToList: "Tüm çalışmalara dön",
    readCase: "Vakayı incele",
  },
  roasTool: {
    eyebrow: "Ücretsiz Araç",
    title: "ROAS ve başabaş noktası hesaplayıcı",
    description:
      "Reklam harcamanı, cironu ve brüt kâr marjını gir; kampanyanın gerçekten kâr edip etmediğini gör. ROAS ciroyu ölçer, kârı değil — asıl eşik marjına bağlıdır.",
    metaTitle: "ROAS Hesaplayıcı — Başabaş ROAS'ı Marjına Göre Hesapla",
    metaDesc:
      "Reklam harcaması, ciro ve kâr marjını gir; ROAS'ını, başabaş ROAS'ını ve net katkını anında gör. Ücretsiz ROAS hesaplama aracı.",
    spend: "Reklam harcaması (aylık)",
    revenue: "Reklamdan gelen ciro",
    margin: "Brüt kâr marjı",
    marginHint: "Ürün maliyeti düşüldükten sonra kalan yüzde",
    yourRoas: "ROAS'ın",
    breakEven: "Başabaş ROAS",
    verdictGood: "Kâr eşiğinin üzerindesin",
    verdictBad: "Kâr eşiğinin altındasın — bu harcama zarar yazıyor",
    grossProfit: "Brüt kâr",
    contribution: "Reklam sonrası net katkı",
    poas: "POAS (kâr bazlı ROAS)",
    poasHint: "Brüt kârın reklam harcamasına oranı. 1.0'ın altı zarar demektir.",
    revenueNeeded: "Başabaş için gereken ciro",
    revenueNeededHint: "Bu harcamanın kendini karşılaması için gereken minimum ciro.",
    ctaText:
      "Rakamların eşiğin altındaysa sorun genelde bütçede değil, huninin kendisindedir. Ücretsiz keşif görüşmesinde birlikte bakalım.",
    ctaButton: "Ücretsiz görüşme",
    faqTitle: "ROAS hakkında sık sorulanlar",
    faq: [
      {
        q: "ROAS ile POAS arasındaki fark nedir?",
        a: "ROAS ciroyu reklam harcamasına böler; ürün maliyetini hesaba katmaz. POAS ise brüt kârı reklam harcamasına böler. Marjı düşük ürünlerde yüksek ROAS bile zarar anlamına gelebilir, POAS bu yanılgıyı ortadan kaldırır.",
      },
      {
        q: "Başabaş ROAS nasıl hesaplanır?",
        a: "Başabaş ROAS = 1 ÷ brüt kâr marjı. Marjın %40 ise 1 ÷ 0,40 = 2,5x. Yani 2,5x'in altındaki her kampanya, raporda pozitif görünse bile kasadan para çıkarır.",
      },
      {
        q: "İyi bir ROAS kaçtır?",
        a: "Tek bir doğru rakam yok; eşik tamamen marjına bağlı. %20 marjlı bir üründe 5x bile başabaştır, %60 marjlı bir üründe 1,7x kâr eder. Sektör ortalamaları yerine kendi eşiğini hesapla.",
      },
      {
        q: "İade ve kargo maliyetlerini nereye koymalıyım?",
        a: "Brüt kâr marjını hesaplarken düşmelisin. İade oranı yüksek kategorilerde (moda gibi) marjı iade sonrası net değerle hesaplamak, gerçek eşiği görmenin tek yoludur.",
      },
    ],
  },
  audit: {
    eyebrow: "Ücretsiz",
    title: "Dijital denetim isteyin",
    description:
      "Sitenizi, reklam hesaplarınızı ve arama görünürlüğünüzü inceleyip nerede para kaybettiğinizi gösteren bir rapor hazırlıyoruz. Karşılığında hiçbir taahhüt istemiyoruz.",
    metaTitle: "Ücretsiz Dijital Pazarlama Denetimi",
    metaDesc:
      "Sitenizi, reklam hesaplarınızı ve arama görünürlüğünüzü inceleyip somut aksiyon listesi çıkarıyoruz. Ücretsiz, taahhütsüz.",
    step1: "Marka",
    step2: "Kanallar",
    step3: "Hedef",
    step4: "İletişim",
    website: "Web siteniz",
    websiteHint: "Denetim bu adres üzerinden yapılacak",
    sector: "Sektörünüz",
    sectors: ["E-ticaret", "Sağlık / Klinik", "Hukuk", "Yerel hizmet", "Sanayi / B2B", "SaaS", "STK", "Diğer"],
    channels: "Şu an aktif olan kanallar",
    channelsHint: "Birden fazla seçebilirsiniz",
    channelOptions: ["Google Ads", "Meta Ads", "SEO", "Sosyal medya", "E-posta", "Hiçbiri"],
    budget: "Aylık reklam bütçeniz",
    budgetOptions: ["Reklam vermiyorum", "0 – 25.000 ₺", "25.000 – 100.000 ₺", "100.000 ₺+"],
    goal: "Önümüzdeki 6 ayda ne olsun istiyorsunuz?",
    goalHint: "Ne kadar somut yazarsanız denetim o kadar isabetli olur",
    goalPlaceholder: "Örnek: Aylık başvuru sayısını iki katına çıkarmak, edinme maliyetini düşürmek…",
    name: "Ad Soyad",
    email: "E-posta",
    phone: "Telefon (opsiyonel)",
    back: "Geri",
    next: "Devam",
    submit: "Denetimi iste",
    sending: "Gönderiliyor…",
    errRequired: "Devam etmek için bu adımı tamamlayın.",
    errSubmit: "Gönderim başarısız. Lütfen tekrar deneyin.",
    summaryWebsite: "Site",
    summarySector: "Sektör",
    summaryChannels: "Aktif kanallar",
    summaryBudget: "Bütçe",
    summaryGoal: "Hedef",
    benefits: [
      "Sitenizin hız ve teknik SEO karnesi",
      "Reklam hesaplarında bütçe kaçağı olan noktalar",
      "Rakiplerin göründüğü, sizin görünmediğiniz aramalar",
      "İlk 90 günde yapılacaklar listesi",
    ],
    promise: "Denetim 3 iş günü içinde e-postanıza gelir. Satış görüşmesi şartı yok.",
  },
  thanks: {
    title: "Talebiniz bize ulaştı",
    description:
      "Denetim raporunuz 3 iş günü içinde e-posta adresinize gelecek. Bu sırada aşağıdakilere göz atabilirsiniz.",
    readBlog: "Blogu incele",
    tryTool: "ROAS hesaplayıcı",
  },
  sectors: {
    eyebrow: "Sektörler",
    title: "Çalıştığımız sektörler",
    description:
      "Her sektörün kendi arama davranışı, mevzuatı ve satın alma süreci var. Aşağıdaki alanlarda gerçek proje deneyimimiz bulunuyor.",
    challengeEyebrow: "Bu sektörün zorlukları",
    challengeTitle: "Nerede tıkanıyor",
    approachEyebrow: "Yaklaşımımız",
    approachTitle: "Nasıl ilerliyoruz",
    casesEyebrow: "Bu sektörden",
    casesTitle: "Çalıştığımız markalar",
    servicesTitle: "Bu sektörde sunulan hizmetler",
    faqTitle: "Sektöre özel sorular",
    otherTitle: "Diğer sektörler",
  },
  brandStrip: { label: "Birlikte büyüdüğümüz markalar" },
  testimonials: {
    eyebrow: "Referanslar",
    title: "Markalar bizimle ne anlatıyor?",
    description:
      "Sayılar etkileyici olabilir; ama bir ajansla çalışmanın asıl değeri, müşterinin sabah uyandığında içinin rahat olmasıdır.",
    prev: "Önceki referans",
    next: "Sonraki referans",
    goTo: "{n}. referans",
  },
  faq: {
    eyebrow: "Sık Sorulanlar",
    title: "Bilmek istediklerinin cevapları",
    description:
      "Aklında olup da burada cevabını bulamadığın bir soru varsa, iletişim sayfasından bize ulaşabilirsin.",
  },
  portfolio: {
    all: "Tümü",
    empty: "Bu kategoride henüz paylaşılacak bir proje yok.",
  },
  servicesPage: {
    gridIntro: "Detayları görmek için bir hizmete tıkla",
    gridLink: "Hangisi sana uygun?",
    fits: "Uygun",
    pricingNote:
      "Sürpriz fatura, gizli kalem, başlangıç ücreti yok. Sözleşme öncesi tüm detaylar yazılı paylaşılır.",
    moreQuestions: "Daha fazla soru için",
  },
  trust: { label: "Onaylar & ortaklıklar" },
  whatsapp: {
    label: "WhatsApp'tan yazın",
    aria: "WhatsApp üzerinden mesaj gönderin",
    // Hazır mesaj: ziyaretçi tuşa bastığında WhatsApp'ta yazılı gelir.
    message:
      "Merhaba, True EDigital Marketing sitesinden yazıyorum. Markam için dijital pazarlama desteği hakkında bilgi almak istiyorum.",
  },
  popup: {
    eyebrow: "Ücretsiz dijital denetim",
    titleTop: "Siteniz nerede",
    titleAccent: "para kaybediyor?",
    titleBottom: "Ücretsiz öğrenin.",
    point1: "Taahhüt yok, satış görüşmesi şartı yok",
    point2: "Rapor 3 iş günü içinde e-postanızda",
    cta: "Ücretsiz denetimi başlat",
    asideTitle: "Denetimde ne var?",
    close: "Kapat",
  },
  chat: {
    title: "EDigital Asistan",
    online: "Çevrimiçi",
    close: "Kapat",
    send: "Gönder",

    greeting: "Merhaba! 👋 Bugün sana nasıl yardımcı olabilirim?",
    subtitleAsk: "Hizmetlerimiz hakkında her şeyi sorabilirsin.",
    placeholder: "Mesajını yaz...",
    error: "Bir sorun oluştu. Lütfen tekrar dene.",
    suggested: [
      "Hangi hizmetleri sunuyorsunuz?",
      "Nasıl görüşme ayarlayabilirim?",
      "Fiyatlandırma nasıl işliyor?",
    ],
    fallback: "Bunu tam bilemedim 🤔 Ekibimiz yardımcı olabilir — İletişim sayfasından bize yazabilir ya da info@etruemarketing.com adresine e-posta gönderebilirsin.",
  },
};

export type Dict = typeof tr;

const en: Dict = {
  nav: {
    home: "Home",
    about: "About",
    services: "Services",
    portfolio: "Portfolio",
    blog: "Blog",
    contact: "Contact",
    allServices: "All services",
    getQuote: "Get a Quote",
    menuOpen: "Open menu",
    menuClose: "Close menu",
    servicesHint: "A single service or a package? Let's decide together.",
  },
  meta: {
    keywords: [
      "digital marketing agency",
      "Google Ads agency",
      "Meta Ads",
      "SEO agency",
      "mobile app development",
      "SaaS development",
      "social media management",
      "performance marketing",
    ],
  },
  common: {
    skipToContent: "Skip to content",
    planCall: "Book a Call",
    getQuoteFree: "Get a Free Quote",
  },
  footer: {
    quickLinks: "Quick Links",
    services: "Services",
    contact: "Contact",
    rights: "All rights reserved.",
    founder: "Founder",
    city: "Istanbul",
  },
  notFound: {
    title: "The page you're looking for wasn't found",
    desc: "It may have moved or never existed. You can continue from the links below.",
    home: "Back to Home",
    contact: "Contact Us",
  },
  contactForm: {
    name: "Full Name",
    namePlaceholder: "Your Full Name",
    email: "Email",
    emailPlaceholder: "you@company.com",
    phone: "Phone (optional)",
    phonePlaceholder: "+90 5XX XXX XX XX",
    service: "Service of Interest",
    serviceSelect: "Select...",
    serviceOther: "Other / Not sure",
    message: "Your Message",
    messagePlaceholder: "Tell us briefly about your project, goals and expectations...",
    privacy: "By clicking Send, you accept our privacy policy.",
    submit: "Send Message",
    submitting: "Sending...",
    successTitle: "Your message reached us!",
    successDesc: "We'll get back to you within 48 hours at the latest.",
    successAgain: "Send another message",
    errName: "Please enter your name (at least 2 characters).",
    errEmail: "Please enter a valid email address.",
    errPhone: "Please enter a valid phone number.",
    errService: "Please choose a service.",
    errMessage: "Please write a more detailed message (at least 20 characters).",
    errSubmit: "Submission failed. Please try again.",
  },
  serviceDetail: {
    crumbHome: "Home",
    crumbServices: "Services",
    eyebrow: "Service",
    getQuote: "Get a Quote for This Service",
    examples: "Examples in This Area",
    idealFor: "Who is it ideal for?",
    approachEyebrow: "Our Approach",
    approachTitle: "How we handle this service",
    scopeEyebrow: "Scope",
    scopeTitle: "What does this service include?",
    scopeDesc: "We don't make promises that aren't in the contract.",
    serviceScope: "Service scope",
    outcomes: "Deliverables",
    expectedResults: "Expected results",
    processEyebrow: "Process",
    processTitle: "From kickoff to first results",
    toolsEyebrow: "Tools",
    toolsTitle: "The stack we use",
    faqEyebrow: "FAQ",
    relatedEyebrow: "Related Services",
    backToAll: "Back to all services",
    breadcrumbAria: "Breadcrumb",
    processDesc: "The steps we go through from the moment we start working together to delivery/launch.",
    toolsDesc: "The platforms we use daily for this service.",
    relatedTitle: "Frequently chosen alongside this service",
    relatedDesc: "You can take it on its own, but the synergy together is far stronger.",
  },
  contactPage: {
    socialMedia: "Social Media",
    dmNote: "Our DM channels are active too — but email is recommended for detailed projects.",
    mapsOpen: "Open in Google Maps",
    officeTitle: "Our office",
    officeMapTitle: "Office location",
  },
  dashboard: {
    label: "Live",
    title: "Campaign Dashboard",
    revenue: "Revenue",
    roas: "ROAS",
    conversions: "Conversions",
    cpa: "CPA",
    traffic: "Performance · last 90 days",
    channels: "Channel split",
  },
  compare: {
    eyebrow: "The difference",
    title: "One nervous system instead of separate agencies",
    description:
      "In the typical setup each agency only watches its own performance. Because we're a single team, cross-channel loss drops to zero.",
    them: "Typical agency setup",
    us: "True EDigital",
    rows: [
      { them: "Separate agencies for ads, SEO, social", us: "All channels in one hand" },
      { them: "Disconnected channels, knowledge silos", us: "One brief, zero cross-channel loss" },
      { them: "Impression and like reports", us: "Sales, lead and ROAS-focused reporting" },
      { them: "Accounts handed to juniors", us: "Founder works on every project directly" },
      { them: "Slow iteration, monthly changes", us: "Tests in 48h, weekly optimization" },
    ],
  },
  blog: {
    nav: "Blog",
    eyebrow: "Blog",
    title: "Notes on digital growth",
    description:
      "Practical writing on ads, SEO and conversion optimization. Not theory — methods that come from the field.",
    readingSuffix: "min read",
    readMore: "Read article",
    empty: "No published articles yet.",
    backToList: "Back to all articles",
    related: "Related articles",
    shareTitle: "Share this article",
    ctaTitle: "Let's discuss this for your brand",
    ctaText: "In a free 30-minute discovery call we review your current channels and put together a concrete plan.",
  },
  caseStudy: {
    client: "Client",
    duration: "Duration",
    headline: "Headline result",
    challenge: "Starting point",
    approach: "Our approach",
    processEyebrow: "Process",
    processTitle: "How we moved",
    resultsEyebrow: "At a glance",
    resultsTitle: "Project at a glance",
    deliverablesEyebrow: "Deliverables",
    deliverablesTitle: "What was produced",
    otherEyebrow: "Other cases",
    otherTitle: "Similar work",
    backToList: "Back to all work",
    readCase: "Read the case",
  },
  roasTool: {
    eyebrow: "Free tool",
    title: "ROAS and break-even calculator",
    description:
      "Enter your ad spend, revenue and gross margin to see whether the campaign is actually profitable. ROAS measures revenue, not profit — the real threshold depends on your margin.",
    metaTitle: "ROAS Calculator — Find Your Break-Even ROAS by Margin",
    metaDesc:
      "Enter ad spend, revenue and profit margin to instantly see your ROAS, break-even ROAS and net contribution. Free ROAS calculator.",
    spend: "Ad spend (monthly)",
    revenue: "Revenue from ads",
    margin: "Gross profit margin",
    marginHint: "What remains after cost of goods",
    yourRoas: "Your ROAS",
    breakEven: "Break-even ROAS",
    verdictGood: "You are above the profit threshold",
    verdictBad: "You are below the profit threshold — this spend loses money",
    grossProfit: "Gross profit",
    contribution: "Net contribution after ads",
    poas: "POAS (profit-based ROAS)",
    poasHint: "Gross profit divided by ad spend. Below 1.0 means a loss.",
    revenueNeeded: "Revenue needed to break even",
    revenueNeededHint: "The minimum revenue required for this spend to pay for itself.",
    ctaText:
      "If your numbers sit below the threshold, the problem is usually the funnel rather than the budget. Let's look at it together in a free call.",
    ctaButton: "Book a free call",
    faqTitle: "Frequently asked about ROAS",
    faq: [
      {
        q: "What is the difference between ROAS and POAS?",
        a: "ROAS divides revenue by ad spend and ignores product cost. POAS divides gross profit by ad spend. On low-margin products a high ROAS can still mean a loss; POAS removes that illusion.",
      },
      {
        q: "How is break-even ROAS calculated?",
        a: "Break-even ROAS = 1 ÷ gross margin. With a 40% margin that is 1 ÷ 0.40 = 2.5x. Any campaign below 2.5x takes money out of the business even when the report looks positive.",
      },
      {
        q: "What counts as a good ROAS?",
        a: "There is no single number — the threshold depends entirely on your margin. At a 20% margin even 5x is break-even; at 60% margin 1.7x is already profitable. Calculate your own threshold instead of chasing industry averages.",
      },
      {
        q: "Where do returns and shipping costs belong?",
        a: "Deduct them when calculating gross margin. In categories with high return rates, such as fashion, using the post-returns margin is the only way to see the real threshold.",
      },
    ],
  },
  audit: {
    eyebrow: "Free",
    title: "Request a digital audit",
    description:
      "We review your site, ad accounts and search visibility, then send you a report showing where money is leaking. No commitment in return.",
    metaTitle: "Free Digital Marketing Audit",
    metaDesc:
      "We review your website, ad accounts and search visibility and hand you a concrete action list. Free, no strings attached.",
    step1: "Brand",
    step2: "Channels",
    step3: "Goal",
    step4: "Contact",
    website: "Your website",
    websiteHint: "The audit will be based on this address",
    sector: "Your sector",
    sectors: ["E-commerce", "Health / Clinic", "Legal", "Local services", "Industry / B2B", "SaaS", "Non-profit", "Other"],
    channels: "Channels currently running",
    channelsHint: "You can pick more than one",
    channelOptions: ["Google Ads", "Meta Ads", "SEO", "Social media", "Email", "None"],
    budget: "Monthly ad budget",
    budgetOptions: ["Not advertising", "€0 – 750", "€750 – 3,000", "€3,000+"],
    goal: "What should change in the next 6 months?",
    goalHint: "The more specific you are, the sharper the audit",
    goalPlaceholder: "Example: double monthly enquiries, bring acquisition cost down…",
    name: "Full name",
    email: "Email",
    phone: "Phone (optional)",
    back: "Back",
    next: "Continue",
    submit: "Request the audit",
    sending: "Sending…",
    errRequired: "Please complete this step to continue.",
    errSubmit: "Submission failed. Please try again.",
    summaryWebsite: "Website",
    summarySector: "Sector",
    summaryChannels: "Active channels",
    summaryBudget: "Budget",
    summaryGoal: "Goal",
    benefits: [
      "Speed and technical SEO scorecard for your site",
      "Where budget leaks in your ad accounts",
      "Searches where competitors show up and you do not",
      "A first-90-days action list",
    ],
    promise: "The audit lands in your inbox within 3 business days. No sales call required.",
  },
  thanks: {
    title: "Your request reached us",
    description:
      "Your audit report will arrive by email within 3 business days. In the meantime, here is something to read.",
    readBlog: "Read the blog",
    tryTool: "ROAS calculator",
  },
  sectors: {
    eyebrow: "Industries",
    title: "Industries we work in",
    description:
      "Every industry has its own search behaviour, regulation and buying process. These are the areas where we have real project experience.",
    challengeEyebrow: "Challenges in this industry",
    challengeTitle: "Where it gets stuck",
    approachEyebrow: "Our approach",
    approachTitle: "How we move",
    casesEyebrow: "From this industry",
    casesTitle: "Brands we have worked with",
    servicesTitle: "Services provided in this industry",
    faqTitle: "Industry-specific questions",
    otherTitle: "Other industries",
  },
  brandStrip: { label: "Brands we've grown with" },
  testimonials: {
    eyebrow: "Testimonials",
    title: "What brands say about working with us",
    description:
      "Numbers can be impressive; but the real value of working with an agency is the client waking up with peace of mind.",
    prev: "Previous testimonial",
    next: "Next testimonial",
    goTo: "Testimonial {n}",
  },
  faq: {
    eyebrow: "FAQ",
    title: "Answers to what you want to know",
    description:
      "If something is on your mind and you can't find the answer here, reach out through the contact page.",
  },
  portfolio: {
    all: "All",
    empty: "No projects to share in this category yet.",
  },
  servicesPage: {
    gridIntro: "Click a service to see the details",
    gridLink: "Which one fits you?",
    fits: "Fits",
    pricingNote:
      "No surprise invoices, no hidden items, no setup fee. Every detail is shared in writing before the contract.",
    moreQuestions: "For more questions",
  },
  trust: { label: "Certifications & partnerships" },
  whatsapp: {
    label: "Message on WhatsApp",
    aria: "Send a message on WhatsApp",
    message:
      "Hello, I am writing from the True EDigital Marketing website. I would like to learn more about digital marketing support for my brand.",
  },
  popup: {
    eyebrow: "Free digital audit",
    titleTop: "Where is your site",
    titleAccent: "losing money?",
    titleBottom: "Find out for free.",
    point1: "No commitment, no sales call required",
    point2: "Report in your inbox within 3 business days",
    cta: "Start the free audit",
    asideTitle: "What the audit covers",
    close: "Close",
  },
  chat: {
    title: "EDigital Assistant",
    online: "Online",
    close: "Close",
    send: "Send",
    greeting: "Hi there! 👋 How can I help you today?",
    subtitleAsk: "Ask anything about our services.",
    placeholder: "Type your message...",
    error: "Something went wrong. Please try again.",
    suggested: [
      "What services do you offer?",
      "How can I book a consultation?",
      "How does pricing work?",
    ],
    fallback: "I'm not sure about that 🤔 Our team can help — write to us via the Contact page or email info@etruemarketing.com.",
  },
};

const de: Dict = {
  nav: {
    home: "Startseite",
    about: "Über uns",
    services: "Leistungen",
    portfolio: "Portfolio",
    blog: "Blog",
    contact: "Kontakt",
    allServices: "Alle Leistungen",
    getQuote: "Angebot anfordern",
    menuOpen: "Menü öffnen",
    menuClose: "Menü schließen",
    servicesHint: "Eine einzelne Leistung oder ein Paket? Entscheiden wir gemeinsam.",
  },
  meta: {
    keywords: [
      "Digitalagentur",
      "Google Ads Agentur",
      "Meta Ads",
      "SEO Agentur",
      "App-Entwicklung",
      "SaaS-Entwicklung",
      "Social-Media-Management",
      "Performance Marketing",
    ],
  },
  common: {
    skipToContent: "Zum Inhalt springen",
    planCall: "Termin buchen",
    getQuoteFree: "Kostenloses Angebot",
  },
  footer: {
    quickLinks: "Schnelllinks",
    services: "Leistungen",
    contact: "Kontakt",
    rights: "Alle Rechte vorbehalten.",
    founder: "Gründer",
    city: "Istanbul",
  },
  notFound: {
    title: "Die gesuchte Seite wurde nicht gefunden",
    desc: "Sie wurde möglicherweise verschoben oder existierte nie. Nutzen Sie die folgenden Links.",
    home: "Zur Startseite",
    contact: "Kontakt aufnehmen",
  },
  contactForm: {
    name: "Vollständiger Name",
    namePlaceholder: "Ihr vollständiger Name",
    email: "E-Mail",
    emailPlaceholder: "sie@firma.com",
    phone: "Telefon (optional)",
    phonePlaceholder: "+90 5XX XXX XX XX",
    service: "Gewünschte Leistung",
    serviceSelect: "Auswählen...",
    serviceOther: "Sonstiges / Nicht sicher",
    message: "Ihre Nachricht",
    messagePlaceholder: "Beschreiben Sie kurz Ihr Projekt, Ihre Ziele und Erwartungen...",
    privacy: "Mit Klick auf Senden akzeptieren Sie unsere Datenschutzrichtlinie.",
    submit: "Nachricht senden",
    submitting: "Wird gesendet...",
    successTitle: "Ihre Nachricht ist angekommen!",
    successDesc: "Wir melden uns spätestens innerhalb von 48 Stunden bei Ihnen.",
    successAgain: "Neue Nachricht senden",
    errName: "Bitte geben Sie Ihren Namen ein (mindestens 2 Zeichen).",
    errEmail: "Bitte geben Sie eine gültige E-Mail-Adresse ein.",
    errPhone: "Bitte geben Sie eine gültige Telefonnummer ein.",
    errService: "Bitte wählen Sie eine Leistung.",
    errMessage: "Bitte schreiben Sie eine ausführlichere Nachricht (mindestens 20 Zeichen).",
    errSubmit: "Übermittlung fehlgeschlagen. Bitte versuchen Sie es erneut.",
  },
  serviceDetail: {
    crumbHome: "Startseite",
    crumbServices: "Leistungen",
    eyebrow: "Leistung",
    getQuote: "Angebot für diese Leistung",
    examples: "Beispiele in diesem Bereich",
    idealFor: "Für wen ist es ideal?",
    approachEyebrow: "Unser Ansatz",
    approachTitle: "Wie wir diese Leistung angehen",
    scopeEyebrow: "Umfang",
    scopeTitle: "Was umfasst diese Leistung?",
    scopeDesc: "Wir machen keine Versprechen, die nicht im Vertrag stehen.",
    serviceScope: "Leistungsumfang",
    outcomes: "Ergebnisse",
    expectedResults: "Erwartbare Ergebnisse",
    processEyebrow: "Ablauf",
    processTitle: "Vom Start bis zum ersten Ergebnis",
    toolsEyebrow: "Tools",
    toolsTitle: "Unser Stack",
    faqEyebrow: "Häufige Fragen",
    relatedEyebrow: "Verwandte Leistungen",
    backToAll: "Zurück zu allen Leistungen",
    breadcrumbAria: "Brotkrumenpfad",
    processDesc: "Die Schritte vom Beginn der Zusammenarbeit bis zur Übergabe bzw. zum Launch.",
    toolsDesc: "Die Plattformen, die wir für diese Leistung täglich nutzen.",
    relatedTitle: "Häufig zusammen mit dieser Leistung gewählt",
    relatedDesc: "Einzeln möglich – gemeinsam ist die Synergie deutlich stärker.",
  },
  contactPage: {
    socialMedia: "Soziale Medien",
    dmNote: "Unsere DM-Kanäle sind ebenfalls aktiv — für detaillierte Projekte empfehlen wir E-Mail.",
    mapsOpen: "In Google Maps öffnen",
    officeTitle: "Unser Büro",
    officeMapTitle: "Bürostandort",
  },
  dashboard: {
    label: "Live",
    title: "Kampagnen-Dashboard",
    revenue: "Umsatz",
    roas: "ROAS",
    conversions: "Conversions",
    cpa: "CPA",
    traffic: "Leistung · letzte 90 Tage",
    channels: "Kanalverteilung",
  },
  compare: {
    eyebrow: "Der Unterschied",
    title: "Ein Nervensystem statt getrennter Agenturen",
    description:
      "Im typischen Setup achtet jede Agentur nur auf ihre eigene Leistung. Weil wir ein einziges Team sind, sinkt der kanalübergreifende Verlust auf null.",
    them: "Typisches Agentur-Setup",
    us: "True EDigital",
    rows: [
      { them: "Getrennte Agenturen für Ads, SEO, Social", us: "Alle Kanäle in einer Hand" },
      { them: "Getrennte Kanäle, Wissenssilos", us: "Ein Briefing, null kanalübergreifender Verlust" },
      { them: "Impressions- und Like-Reports", us: "Auf Umsatz, Leads und ROAS fokussiertes Reporting" },
      { them: "Accounts an Junioren übergeben", us: "Gründer arbeitet an jedem Projekt selbst" },
      { them: "Langsame Iteration, monatliche Änderungen", us: "Tests in 48 Std., wöchentliche Optimierung" },
    ],
  },
  blog: {
    nav: "Blog",
    eyebrow: "Blog",
    title: "Notizen zu digitalem Wachstum",
    description:
      "Praxisnahe Beiträge zu Ads, SEO und Conversion-Optimierung. Keine Theorie — Methoden aus der Praxis.",
    readingSuffix: "Min. Lesezeit",
    readMore: "Beitrag lesen",
    empty: "Noch keine veröffentlichten Beiträge.",
    backToList: "Zurück zu allen Beiträgen",
    related: "Verwandte Beiträge",
    shareTitle: "Diesen Beitrag teilen",
    ctaTitle: "Sprechen wir darüber für Ihre Marke",
    ctaText: "In einem kostenlosen 30-minütigen Erstgespräch prüfen wir Ihre Kanäle und erstellen einen konkreten Plan.",
  },
  caseStudy: {
    client: "Kunde",
    duration: "Dauer",
    headline: "Zentrales Ergebnis",
    challenge: "Ausgangslage",
    approach: "Unser Ansatz",
    processEyebrow: "Ablauf",
    processTitle: "So sind wir vorgegangen",
    resultsEyebrow: "Auf einen Blick",
    resultsTitle: "Projekt auf einen Blick",
    deliverablesEyebrow: "Ergebnisse im Detail",
    deliverablesTitle: "Was im Projekt entstanden ist",
    otherEyebrow: "Weitere Fälle",
    otherTitle: "Ähnliche Projekte",
    backToList: "Zurück zu allen Projekten",
    readCase: "Fallstudie lesen",
  },
  roasTool: {
    eyebrow: "Kostenloses Tool",
    title: "ROAS- und Break-even-Rechner",
    description:
      "Geben Sie Werbeausgaben, Umsatz und Rohmarge ein und sehen Sie, ob die Kampagne tatsächlich profitabel ist. ROAS misst Umsatz, nicht Gewinn — die eigentliche Schwelle hängt von Ihrer Marge ab.",
    metaTitle: "ROAS-Rechner — Break-even-ROAS nach Marge berechnen",
    metaDesc:
      "Werbeausgaben, Umsatz und Marge eingeben und sofort ROAS, Break-even-ROAS und Deckungsbeitrag sehen. Kostenloser ROAS-Rechner.",
    spend: "Werbeausgaben (monatlich)",
    revenue: "Umsatz aus Werbung",
    margin: "Rohertragsmarge",
    marginHint: "Was nach Wareneinsatz übrig bleibt",
    yourRoas: "Ihr ROAS",
    breakEven: "Break-even-ROAS",
    verdictGood: "Sie liegen über der Gewinnschwelle",
    verdictBad: "Sie liegen unter der Gewinnschwelle — diese Ausgaben sind ein Verlust",
    grossProfit: "Rohertrag",
    contribution: "Deckungsbeitrag nach Werbung",
    poas: "POAS (gewinnbasierter ROAS)",
    poasHint: "Rohertrag geteilt durch Werbeausgaben. Unter 1,0 bedeutet Verlust.",
    revenueNeeded: "Umsatz bis zum Break-even",
    revenueNeededHint: "Der Mindestumsatz, damit sich diese Ausgaben selbst tragen.",
    ctaText:
      "Liegen Ihre Zahlen unter der Schwelle, liegt das Problem meist im Funnel und nicht im Budget. Schauen wir gemeinsam darauf.",
    ctaButton: "Kostenloses Gespräch",
    faqTitle: "Häufige Fragen zum ROAS",
    faq: [
      {
        q: "Was ist der Unterschied zwischen ROAS und POAS?",
        a: "ROAS teilt den Umsatz durch die Werbeausgaben und ignoriert die Produktkosten. POAS teilt den Rohertrag durch die Werbeausgaben. Bei margenschwachen Produkten kann ein hoher ROAS trotzdem Verlust bedeuten; POAS beseitigt diesen Trugschluss.",
      },
      {
        q: "Wie wird der Break-even-ROAS berechnet?",
        a: "Break-even-ROAS = 1 ÷ Rohmarge. Bei 40 % Marge sind das 1 ÷ 0,40 = 2,5x. Jede Kampagne unter 2,5x kostet Geld, auch wenn der Report positiv aussieht.",
      },
      {
        q: "Was ist ein guter ROAS?",
        a: "Es gibt keine allgemeingültige Zahl — die Schwelle hängt vollständig von Ihrer Marge ab. Bei 20 % Marge ist selbst 5x nur Break-even, bei 60 % Marge ist 1,7x bereits profitabel.",
      },
      {
        q: "Wohin gehören Retouren und Versandkosten?",
        a: "Sie gehören in die Berechnung der Rohmarge. In Kategorien mit hoher Retourenquote, etwa Mode, zeigt nur die Marge nach Retouren die echte Schwelle.",
      },
    ],
  },
  audit: {
    eyebrow: "Kostenlos",
    title: "Digital-Audit anfordern",
    description:
      "Wir prüfen Ihre Website, Werbekonten und Suchsichtbarkeit und senden Ihnen einen Bericht, der zeigt, wo Budget verloren geht. Ohne Gegenleistung.",
    metaTitle: "Kostenloses Digital-Marketing-Audit",
    metaDesc:
      "Wir prüfen Website, Werbekonten und Suchsichtbarkeit und liefern eine konkrete Maßnahmenliste. Kostenlos und unverbindlich.",
    step1: "Marke",
    step2: "Kanäle",
    step3: "Ziel",
    step4: "Kontakt",
    website: "Ihre Website",
    websiteHint: "Das Audit basiert auf dieser Adresse",
    sector: "Ihre Branche",
    sectors: ["E-Commerce", "Gesundheit / Klinik", "Recht", "Lokale Dienstleistungen", "Industrie / B2B", "SaaS", "Non-Profit", "Sonstiges"],
    channels: "Aktuell laufende Kanäle",
    channelsHint: "Mehrfachauswahl möglich",
    channelOptions: ["Google Ads", "Meta Ads", "SEO", "Social Media", "E-Mail", "Keine"],
    budget: "Monatliches Werbebudget",
    budgetOptions: ["Keine Werbung", "0 – 750 €", "750 – 3.000 €", "3.000 €+"],
    goal: "Was soll sich in den nächsten 6 Monaten ändern?",
    goalHint: "Je konkreter, desto präziser das Audit",
    goalPlaceholder: "Beispiel: Anfragen pro Monat verdoppeln, Akquisekosten senken…",
    name: "Vor- und Nachname",
    email: "E-Mail",
    phone: "Telefon (optional)",
    back: "Zurück",
    next: "Weiter",
    submit: "Audit anfordern",
    sending: "Wird gesendet…",
    errRequired: "Bitte vervollständigen Sie diesen Schritt.",
    errSubmit: "Übermittlung fehlgeschlagen. Bitte erneut versuchen.",
    summaryWebsite: "Website",
    summarySector: "Branche",
    summaryChannels: "Aktive Kanäle",
    summaryBudget: "Budget",
    summaryGoal: "Ziel",
    benefits: [
      "Tempo- und Technik-SEO-Bewertung Ihrer Website",
      "Wo in Ihren Werbekonten Budget verloren geht",
      "Suchanfragen, bei denen Wettbewerber erscheinen und Sie nicht",
      "Maßnahmenliste für die ersten 90 Tage",
    ],
    promise: "Das Audit erreicht Sie innerhalb von 3 Werktagen per E-Mail. Kein Verkaufsgespräch nötig.",
  },
  thanks: {
    title: "Ihre Anfrage ist angekommen",
    description:
      "Ihr Audit-Bericht erreicht Sie innerhalb von 3 Werktagen per E-Mail. In der Zwischenzeit finden Sie hier Lesestoff.",
    readBlog: "Zum Blog",
    tryTool: "ROAS-Rechner",
  },
  sectors: {
    eyebrow: "Branchen",
    title: "Branchen, in denen wir arbeiten",
    description:
      "Jede Branche hat ihr eigenes Suchverhalten, ihre Regulierung und ihren Kaufprozess. In diesen Bereichen haben wir echte Projekterfahrung.",
    challengeEyebrow: "Herausforderungen dieser Branche",
    challengeTitle: "Wo es hakt",
    approachEyebrow: "Unser Ansatz",
    approachTitle: "So gehen wir vor",
    casesEyebrow: "Aus dieser Branche",
    casesTitle: "Marken, mit denen wir gearbeitet haben",
    servicesTitle: "Leistungen in dieser Branche",
    faqTitle: "Branchenspezifische Fragen",
    otherTitle: "Weitere Branchen",
  },
  brandStrip: { label: "Marken, mit denen wir gemeinsam gewachsen sind" },
  testimonials: {
    eyebrow: "Referenzen",
    title: "Was Marken über die Zusammenarbeit sagen",
    description:
      "Zahlen können beeindrucken; der eigentliche Wert einer Agentur zeigt sich aber daran, dass Kundinnen und Kunden morgens beruhigt aufwachen.",
    prev: "Vorherige Referenz",
    next: "Nächste Referenz",
    goTo: "Referenz {n}",
  },
  faq: {
    eyebrow: "Häufige Fragen",
    title: "Antworten auf Ihre Fragen",
    description:
      "Wenn Sie hier keine Antwort finden, schreiben Sie uns einfach über die Kontaktseite.",
  },
  portfolio: {
    all: "Alle",
    empty: "In dieser Kategorie gibt es noch keine Projekte.",
  },
  servicesPage: {
    gridIntro: "Klicken Sie auf eine Leistung, um Details zu sehen",
    gridLink: "Welche passt zu Ihnen?",
    fits: "Passend für",
    pricingNote:
      "Keine Überraschungsrechnungen, keine versteckten Posten, keine Einrichtungsgebühr. Alle Details werden vor Vertragsabschluss schriftlich geteilt.",
    moreQuestions: "Für weitere Fragen",
  },
  trust: { label: "Zertifizierungen & Partnerschaften" },
  whatsapp: {
    label: "Per WhatsApp schreiben",
    aria: "Nachricht per WhatsApp senden",
    message:
      "Hallo, ich schreibe von der Website True EDigital Marketing. Ich möchte mehr über digitale Marketing-Unterstützung für meine Marke erfahren.",
  },
  popup: {
    eyebrow: "Kostenloses Digital-Audit",
    titleTop: "Wo verliert Ihre Website",
    titleAccent: "Geld?",
    titleBottom: "Kostenlos herausfinden.",
    point1: "Keine Verpflichtung, kein Verkaufsgespräch nötig",
    point2: "Bericht in 3 Werktagen in Ihrem Postfach",
    cta: "Kostenloses Audit starten",
    asideTitle: "Was das Audit prüft",
    close: "Schließen",
  },
  chat: {
    title: "EDigital Assistent",
    online: "Online",
    close: "Schließen",
    send: "Senden",
    greeting: "Hallo! 👋 Wie kann ich Ihnen heute helfen?",
    subtitleAsk: "Fragen Sie alles über unsere Leistungen.",
    placeholder: "Ihre Nachricht...",
    error: "Etwas ist schiefgelaufen. Bitte erneut versuchen.",
    suggested: [
      "Welche Leistungen bieten Sie an?",
      "Wie kann ich ein Gespräch buchen?",
      "Wie funktioniert die Preisgestaltung?",
    ],
    fallback: "Das weiß ich nicht genau 🤔 Unser Team hilft Ihnen gern — schreiben Sie uns über die Kontakt-Seite oder per E-Mail an info@etruemarketing.com.",
  },
};

const DICTS: Record<Locale, Dict> = { tr, en, de };

export function getDict(locale: Locale): Dict {
  return DICTS[locale] ?? tr;
}
